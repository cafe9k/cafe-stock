...
const CNINFO_API_URL = "https://www.cninfo.com.cn/new/hisAnnouncement/query";
const CNINFO_PDF_BASE_URL = "https://static.cninfo.com.cn/";
...
function checkRateLimit(ip: string): { error: string; waitMs: number } | null {
	...
	if (tracker.requests.length >= RATE_LIMIT_CONFIG.maxRequestsPerMinute) {
		...
		return {
			error: `请求频率超限: ${tracker.requests.length}/${RATE_LIMIT_CONFIG.maxRequestsPerMinute} 次/分钟，请等待 ${Math.ceil(
				waitMs / 1000
			)} 秒`,
			waitMs,
		};
...
function getRateLimitHeaders(ip: string): Record<string, string> {
	...
	const resetTime =
		tracker.requests.length > 0 ? Math.ceil((tracker.requests[0] + RATE_LIMIT_CONFIG.windowMs) / 1000) : Math.ceil(Date.now() / 1000) + 60;
...
console.log(
	`✅ CNInfo Proxy started with rate limiting: min interval ${RATE_LIMIT_CONFIG.minIntervalMs}ms, max ${RATE_LIMIT_CONFIG.maxRequestsPerMinute} req/min`
);
...
serve(async (req) => {
	...
	try {
		...
		console.log(`[Proxy] Request params: ${params.toString()}`);
		...
		if (!response.ok) {
			...
			console.error(`[Error] CNInfo API returned ${response.status}: ${errorText}`);
			throw new Error(`CNInfo API returned ${response.status}`);
		}

		const rawData = await response.json();
		console.log(`[Proxy] Response totalRecordNum: ${rawData.totalRecordNum}, announcements count: ${rawData.announcements?.length || 0}`);
