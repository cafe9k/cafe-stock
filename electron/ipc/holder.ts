/**
 * 依赖: ipcMain(Electron), holderService(服务层), TushareClient(API), StockRepository(数据访问)
 * 输出: registerHolderHandlers() - 注册股东相关的IPC处理器（get-top10-holders, sync-holders等）
 * 职责: IPC通信层股东处理器，连接渲染进程与主进程的股东业务逻辑
 *
 * ⚠️ 更新提醒：修改此文件后，请同步更新：
