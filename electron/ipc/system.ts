/**
 * 依赖: ipcMain(Electron), Notification(通知), shell(系统shell), updater(更新器), errorHandler(错误处理)
 * 输出: registerSystemHandlers() - 注册系统相关的IPC处理器（show-notification, get-app-version等）
 * 职责: IPC通信层系统处理器，提供系统级功能（通知、版本、外部链接等）
 *
 * ⚠️ 更新提醒：修改此文件后，请同步更新：
