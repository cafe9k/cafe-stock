...
import { upsertStocks, getAllStocks, countStocks, searchStocks, addFavoriteStock, removeFavoriteStock, isFavoriteStock, getAllFavoriteStocks, countFavoriteStocks, upsertTop10Holders, getTop10HoldersByStock, hasTop10HoldersData, getStocksWithTop10Holders, countStocksWithTop10Holders, deleteTop10HoldersByStock, getTop10HoldersEndDates, getTop10HoldersByStockAndEndDate, getDbPath, getStockListSyncInfo, updateSyncFlag, isSyncedToday, getCacheDataStats, upsertAnnouncements, isAnnouncementRangeSynced, getUnsyncedAnnouncementRanges, recordAnnouncementSyncRange, getAnnouncementsByDateRange, searchAnnouncements, countAnnouncements, } from "./db.js";
...
async function getAnnouncementsGroupedFromAPI(page, pageSize, startDate, endDate, market) {
    ...
    if (startDate && endDate) {
        ...
        announcements = await TushareClient.getAnnouncementsComplete(undefined, // 全市场
        startDate, endDate, (message, current, total) => {
...
function setupIPC() {
    ...
    ipcMain.handle("get-announcements-grouped", async (_event, page, pageSize, startDate, endDate, market) => {
        try {
            console.log(`[IPC] get-announcements-grouped: page=${page}, pageSize=${pageSize}, dateRange=${startDate}-${endDate}, market=${market}`);
    ...
    ipcMain.handle("search-announcements-grouped", async (_event, keyword, page, pageSize, startDate, endDate, market) => {
        try {
            console.log(`[IPC] search-announcements-grouped: keyword=${keyword}, page=${page}, pageSize=${pageSize}, dateRange=${startDate}-${endDate}, market=${market}`);
            const result = await searchAnnouncementsGroupedFromAPI(keyword, page, pageSize, startDate, endDate, market);
            console.log(`[IPC] search-announcements-grouped: keyword=${keyword}, page=${page}, items=${result.items.length}, total=${result.total}`);
            return {
                items: result.items,
                total: result.total,
                page,
    ...
    ipcMain.handle("start-sqlite-http-server", async (_event, port) => {
        try {
            ...
            sqliteHttpServer.on("error", (error) => {
    ...
    ipcMain.handle("get-favorite-stocks-announcements-grouped", async (_event, page, pageSize, startDate, endDate) => {
        try {
    ...
    // 获取财经资讯
    ipcMain.handle("get-news", async (_event, src, startDate, endDate) => {
        try {
    ...
    ipcMain.handle("get-top10-holders", async (_event, tsCode, period, annDate, startDate, endDate) => {
        try {
            console.log(`[IPC] get-top10-holders: tsCode=${tsCode}, period=${period}, annDate=${annDate}, startDate=${startDate}, endDate=${endDate}`);
     ...
     */
    ipcMain.handle("get-announcements-with-cache", async (_event, tsCode, startDate, endDate, onProgressCallback) => {
        try {
            ...
            console.log(`[IPC] 需要同步 ${unsyncedRanges.length} 个时间段:`, unsyncedRanges.map((r) => `${r.start_date}-${r.end_date}`));
            // 对每个未同步的时间段，迭代获取数据
            for (const range of unsyncedRanges) {
                console.log(`[IPC] 开始同步时间段: ${range.start_date} - ${range.end_date}`);
                // 使用迭代方法获取该时间段的所有公告
                const announcements = await TushareClient.getAnnouncementsIterative(tsCode || undefined, range.start_date, range.end_date, onProgressCallback
                    ? (currentCount, totalFetched) => {
                        // 发送进度给渲染进程
