...
export default function DashboardPage() {
	...
	// 消息系统 hook
	const { alerts, unreadCount, loading: alertsLoading, scanning, scanAlerts, markAsRead, markAllAsRead, deleteAlert } = useStockAlerts();
		...
		<div className="dashboard">
			...
			<header className="dashboard-header">
				...
				<div className="header-right">
					<button className="btn-alerts" onClick={() => setShowAlertPanel(true)} title="消息中心">
			...
			{activeTab === "panel" && (
				<>
					{/* 消息横幅 */}
					<AlertBanner alerts={alerts} unreadCount={unreadCount} onViewAll={() => setShowAlertPanel(true)} onMarkAsRead={markAsRead} />
					...
					{quotesError && (
						<div className="error-banner">
							...
							<span className="error-text">
								{quotesError.includes("IP数量超限") ? "行情数据获取受限，Tushare API IP 限制，请稍后重试" : quotesError}
							</span>
							...
							<div className="stock-grid">
								{sortedStocks.length > 0 ? (
									...
									<div className="empty-state">
										...
										<h3>{selectedGroupId ? "该分组还没有股票" : "还没有关注的股票"}</h3>
										<p>点击"添加股票"开始追踪您感兴趣的股票</p>
										<button className="btn-add-stock-large" onClick={() => setShowAddModal(true)}>
											+ 添加第一只股票
										</button>
									</div>
								)}
			...
			{/* 添加股票模态框 */}
			<AddStockModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} onAdd={handleAddStock} isStockWatched={isStockWatched} />

			{/* 股票详情面板 */}
			{selectedStock && (
				<StockDetailPanel stock={selectedStock} quote={getQuote(selectedStock.ts_code)} onClose={() => setSelectedStock(null)} />
			)}
